...
Run =M-x nerd-icons-install-fonts= to install Nerd Font symbols to the Font book. This makes all icons work inside Dired.
...
NOTE: If tangling doesn't work, check that the symlink from above is correct.
...
Insert Swedish characters åäö by typing aa ae oe. Toggle this using =C-/= (toggle-input-method).
...
- =C-; E= :: =embark-export= to get a buffer of search results. This can be filtered using =consult-keep-lines=.
- =C-; S= :: =embark-collect= displays a list of results
...
- =g O= :: Open the file in a vertical split
- =C-; o= :: Open a file from Vertico (search like =SPC SPC=) in a vertical split. =C-;= is an action menu using Embark.
- =SPC m h= :: Toggle showing hidden files (omit mode)
- =M-x consult-keep-lines= :: Filter outputs, for example from a grep out (like =hl-todo-rgrep=)
...
To make Dired sort folders first, install =brew install coreutils= and set it on the path. This use GNU based tools instead of Mac's BSD based.

*** File handling

Create new files with =C-x C-f=. Rename files by pressing =i=, then edit the buffer and save.
